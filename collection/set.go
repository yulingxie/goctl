...
func (s *Set) validate(i interface{}) {
	...
	case int:
		if s.tp != intType {
			fmt.Printf("Error: element is int, but set contains elements with type %d", s.tp)
		}
	case int64:
		if s.tp != int64Type {
			fmt.Printf("Error: element is int64, but set contains elements with type %d", s.tp)
		}
	case uint:
		if s.tp != uintType {
			fmt.Printf("Error: element is uint, but set contains elements with type %d", s.tp)
		}
	case uint64:
		if s.tp != uint64Type {
			fmt.Printf("Error: element is uint64, but set contains elements with type %d", s.tp)
		}
	case string:
		if s.tp != stringType {
			fmt.Printf("Error: element is string, but set contains elements with type %d", s.tp)
		}
	}
}
