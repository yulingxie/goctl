...
import (
	...
	"gitlab.kaiqitech.com/k7game/server/tools/goctl/vars"
)
...
func Run(arg string, dir string, in ...*bytes.Buffer) (string, error) {
	...
	if err != nil {
		if stderr.Len() > 0 {
			return "", errors.New(strings.TrimSuffix(stderr.String(), vars.NL))
		}
		return "", err
	}

	return strings.TrimSuffix(stdout.String(), vars.NL), nil
}
