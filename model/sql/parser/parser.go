...
import (
	...
	"gitlab.kaiqitech.com/k7game/server/tools/goctl/collection"
	"gitlab.kaiqitech.com/k7game/server/tools/goctl/model/sql/converter"
...
func convertColumns(columns []*sqlparser.ColumnDefinition, primaryColumn string) (Primary, map[string]*Field, error) {
	...
	for _, column := range columns {
		...
		dataType, err := converter.ConvertDataType2(column.Type.Type, bool(column.Type.Unsigned), isDefaultNull)
		if err != nil {
...
func ConvertDataType(tableData *model.TableData) (*Table, error) {
	isPrimaryDefaultNull := tableData.PrimaryKey.ColumnDefault == nil && tableData.PrimaryKey.IsNullAble == "YES"
	primaryDataType, err := converter.ConvertDataType(tableData.PrimaryKey.DataType, tableData.PrimaryKey.ColumnType, isPrimaryDefaultNull)
	if err != nil {
		return nil, err
	}

	var reply Table
	reply.DbName = tableData.Db
	reply.UniqueIndex = map[string][]*Field{}
	reply.NormalIndex = map[string][]*Field{}
	reply.Name = stringx.From(tableData.Table)
	seqInIndex := 0
	if tableData.PrimaryKey.Statistic != nil {
		seqInIndex = tableData.PrimaryKey.SeqInIndex
	}

	reply.PrimaryKey = Primary{
		Field: Field{
			Name:            stringx.From(tableData.PrimaryKey.Name),
			DataBaseType:    tableData.PrimaryKey.DataType,
			DataType:        primaryDataType,
			Comment:         tableData.PrimaryKey.Comment,
			SeqInIndex:      seqInIndex,
			OrdinalPosition: tableData.PrimaryKey.OrdinalPosition,
		},
		AutoIncrement: strings.Contains(tableData.PrimaryKey.Extra, "auto_increment"),
	}

	fieldM, err := getTableFields(tableData)
	if err != nil {
		return nil, err
	}
	...
	log := console.NewColorConsole()
	for indexName, each := range tableData.UniqueIndex {
		sort.Slice(each, func(i, j int) bool {
			if each[i].Statistic != nil && each[j].Statistic != nil {
		...
		if len(each) == 1 {
			one := each[0]
			if one.Name == tableData.PrimaryKey.Name {
				log.Warning("table %s: duplicate unique index with primary key, %s", tableData.Table, one.Name)
				continue
			}
		}
		...
		if uniqueIndexSet.Contains(uniqueKey) {
			log.Warning("table %s: duplicate unique index, %s", tableData.Table, uniqueKey)
			continue
		}
	...
	normalIndexSet := collection.NewSet()
	for indexName, each := range tableData.NormalIndex {
		var list []*Field
		...
		if normalIndexSet.Contains(normalKey) {
			log.Warning("table %s: duplicate index, %s", tableData.Table, normalKey)
			continue
		}
...
func getTableFields(tableData *model.TableData) (map[string]*Field, error) {
	fieldM := make(map[string]*Field)
	for _, each := range tableData.ColumnDatas {
		isDefaultNull := each.ColumnDefault == nil && each.IsNullAble == "YES"
		dt, err := converter.ConvertDataType(each.DataType, each.ColumnType, isDefaultNull)
		if err != nil {
			return nil, err
		}
		columnSeqInIndex := 0
		if each.Statistic != nil {
			columnSeqInIndex = each.SeqInIndex
		}
